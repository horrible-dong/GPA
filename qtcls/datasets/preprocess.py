...
def preprocess_g_internet(args, raw_dir, processed_dir, processed_file):
...
def preprocess_elliptic(args, raw_dir, processed_dir, processed_file):
    ...
    graph.ndata['centrality'] = graph.in_degrees()
    behaviors = bin_encoding(graph, mask_to_index(graph.ndata['train_mask']), n_bin=args.n_bin)
    graph.ndata['behav'] = torch.FloatTensor(behaviors).contiguous()
